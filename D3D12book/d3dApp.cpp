...
bool D3DApp::InitMainWindow() {
	...
	mhMainWnd = CreateWindow(L"MainWnd", mMainWndCaption.c_str(), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, width, height, 0, 0, wc.hInstance, 0);
...
void D3DApp::FlushCommandQueue() {
	...
	//CommandQueue 끝에 Fence의 값을 mCurrentFence로 바꾸는 명령어 넣기
	ThrowIfFailed(mCommandQueue->Signal(mFence.Get(), mCurrentFence));

	//값이 mCurrentFence값 이상이면 이미 실행한 이후이므로 통과
	//이하라면 아직 Signal로 넣은 명령어까지 실행하지 못했다는 뜻이므로
	//Event를 생성해서 바뀔 때까지 대기하기

...
void LitWavesApp::OnMouseDown(WPARAM wParam, int x, int y) {
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void LitWavesApp::OnMouseMove(WPARAM btnState, int x, int y) {
	if ((btnState & MK_LBUTTON) != 0) {
		...
		float dx = XMConvertToRadians(2.2f * static_cast<float>(x - mLastMousePos.x));
		float dy = XMConvertToRadians(2.2f * static_cast<float>(y - mLastMousePos.y));
...
void LitWavesApp::UpdateMainPassCB(const GameTimer& gt) {
	...
	float blinkStrength = abs(sinf(gt.TotalTime() * 5.0f));

	mMainPassCB.Lights[0].Strength = { blinkStrength, 0.0f, 0.0f};
...
void LitWavesApp::BuildLandGeometry() {
	...
	std::vector<WaveVertex> vertices(grid.Vertices.size());
	...
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(WaveVertex);

	std::vector<std::uint16_t> indices = grid.GetIndices16();
	const UINT ibByteSize = (UINT)indices.size() * sizeof(WaveVertex);
	...
	geo->VertexByteStride = sizeof(WaveVertex);
...
void LitWavesApp::BuildWavesGeometryBuffers() {
	...
	UINT vbByteSize = mWaves->VertexCount() * sizeof(WaveVertex);
...
void LitWavesApp::BuildRenderItems() {
	auto wavesRitem = std::make_unique<WaveRenderItem>();
	...
	auto gridRitem = std::make_unique<WaveRenderItem>();
...
void LitWavesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<WaveRenderItem*>& ritems) {

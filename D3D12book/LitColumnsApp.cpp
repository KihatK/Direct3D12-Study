...
LitColumnsApp::LitColumnsApp(HINSTANCE hInstace) {
...
void LitColumnsApp::Draw(const GameTimer& gt) {
	...
	mCommandList->OMSetRenderTargets(1, &currentBackBuffer, true, &depthStencilView);
	...
	auto barrierToPR = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
...
void LitColumnsApp::UpdateMainPassCB(const GameTimer& gt) {
	...
	auto invViewDet = XMMatrixDeterminant(view);
	XMMATRIX invView = XMMatrixInverse(&invViewDet, view);
	auto invProjDet = XMMatrixDeterminant(proj);
	XMMATRIX invProj = XMMatrixInverse(&invProjDet, proj);
	auto invViewProjDet = XMMatrixDeterminant(viewProj);
	XMMATRIX invViewProj = XMMatrixInverse(&invViewProjDet, viewProj);
	...
	for (int i = 3; i < 13; i++) {
		mMainPassCB.Lights[i].Strength = { 0.8f, 0.8f, 0.6f };
		mMainPassCB.Lights[i].Direction = { 0.0f, -1.0f, 0.0f };
		mMainPassCB.Lights[i].FalloffStart = 1.0f;
		mMainPassCB.Lights[i].FalloffEnd = 15.0f;
		mMainPassCB.Lights[i].SpotPower = 10.0f;
	}
...
void LitColumnsApp::BuildShadersAndInputLayout() {
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"LitDefault.hlsl", alphaTestDefines, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"LitDefault.hlsl", alphaTestDefines, "PS", "ps_5_1");
...
void LitColumnsApp::BuildShapeGeometry() {
	...
	std::vector<LitVertex> vertices(totalVertexCount);
	...
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(LitVertex);
	...
	geo->VertexByteStride = sizeof(LitVertex);
...
void LitColumnsApp::BuildSkullGeometry() {
	...
	std::vector<LitVertex> vertices(vcount);
...
void LitColumnsApp::BuildRenderItems() {
	auto boxRitem = std::make_unique<LitRenderItem>();
	...
	auto gridRitem = std::make_unique<LitRenderItem>();
	...
	auto skullRitem = std::make_unique<LitRenderItem>();
	...
	for (int i = 0; i < 5; ++i)
	{
		auto leftCylRitem = std::make_unique<LitRenderItem>();
		auto rightCylRitem = std::make_unique<LitRenderItem>();
		auto leftSphereRitem = std::make_unique<LitRenderItem>();
		auto rightSphereRitem = std::make_unique<LitRenderItem>();
		...
		mMainPassCB.Lights[(2 * i) + 3].Position = XMFLOAT3(-5.0f, 3.5f, -10.0f + i * 5.0f);
		mMainPassCB.Lights[(2 * i + 1) + 3].Position = XMFLOAT3(+5.0f, 3.5f, -10.0f + i * 5.0f);
...
void LitColumnsApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<LitRenderItem*>& ritems) {

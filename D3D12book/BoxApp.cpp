...
void BoxApp::Update(const GameTimer& gt) {
	...
	objConstants.gPulseColor = XMFLOAT4(Colors::Red);
...
void BoxApp::Draw(const GameTimer& gt) {
	...
	auto transitionToRT = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
	...
	cbv1.Offset(1, mCbvSrvUavDescriptorSize);
	mCommandList->SetGraphicsRootDescriptorTable(0, cbv1);
	...
	auto transitionToPresent = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
...
void BoxApp::BuildConstantBuffers() {
	mObjectCB = std::make_unique<UploadBuffer<BoxObjectConstants>>(md3dDevice.Get(), 2, true);

	UINT objCBByteSize = d3dUtil::CalcConstantBufferByteSize(sizeof(BoxObjectConstants));
...
void BoxApp::BuildBoxGeometry() {
	std::array<BoxVertex, 8> boxVertices = {
	...
	std::array<BoxVertex, 5> pyramidVertices = {
		BoxVertex({ XMFLOAT3(-1.0f, -1.0f, -1.0f), XMFLOAT4(Colors::Green) }),//V0
		BoxVertex({ XMFLOAT3(-1.0f, -1.0f, +1.0f), XMFLOAT4(Colors::Green) }),//V1
		BoxVertex({ XMFLOAT3(+1.0f, -1.0f, -1.0f), XMFLOAT4(Colors::Green) }),//V2
		BoxVertex({ XMFLOAT3(+1.0f, -1.0f, +1.0f), XMFLOAT4(Colors::Green) }),//V3
		BoxVertex({ XMFLOAT3(0.0f, +2.0f,  0.0f), XMFLOAT4(Colors::Red) }) //V4
	};
	std::array<BoxVertex, boxVertices.size() + pyramidVertices.size()> vertices;
	...
	pyramidSubmesh.StartIndexLocation = boxIndices.size();
	pyramidSubmesh.BaseVertexLocation = boxVertices.size();
